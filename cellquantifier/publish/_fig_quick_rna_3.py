...
def fig_quick_rna_3(merged_blobs_df, merged_int_df, norm_row_props,
	...
	count_df = merged_blobs_df.groupby(['label','cell_type', 'prefix',\
										'region_type', 'sample_type'\
										]).size().reset_index(name="count")

	# count_df.to_csv('/home/clayton/Desktop/data-analysis/200521_HLA-DMB-final-analysis/200521_HLA-DMB-count-df.csv')

	# """
	# ~~~~~~~~~~~Copy number group bar chart~~~~~~~~~~~~~~
	# """

	data_col = 'count'; cat_cols = ['cell_type', 'sample_type']
	...
	# count_df_stat.to_csv('/home/clayton/Desktop/data-analysis/200521_HLA-DMB-final-analysis/200521_HLA-DMB-count-df-stat.csv')
	...
	ax[0].legend(custom_lines, sorted(count_df['sample_type'].unique()))
	...
	for i, this_df in enumerate(df_arr):
		p_val_df = get_pval_df(this_df, data_col='count', cat_col='sample_type')
		p_val_arr = [p_val_df.at['ctrl', 'aabp'], p_val_df.at['ctrl', 't1d']]
		add_stat_anno(ax[0],
					  x_arr=ind_arr[i],
	...
	region_type_arr = merged_blobs_df['region_type'].unique()
	label_map = merged_blobs_df[['label', 'sample_type']].drop_duplicates('label')
	...
	# frac_df.to_csv('/home/clayton/Desktop/data-analysis/200521_HLA-DMB-final-analysis/200521_HLA-DMB-frac-df.csv')

	stack_col = 'region_type'
	data_col = 'fracs'; cat_cols = ['cell_type', 'sample_type']
	...
	# frac_df_stat.to_csv('/home/clayton/Desktop/data-analysis/200521_HLA-DMB-final-analysis/200521_HLA-DMB-frac-df-stat.csv')
	...
	df_arr = nest_df(frac_df, cat_cols=['cell_type', 'sample_type'])
	p_val_arr = df_to_pval(df_arr, cat_col='region_type', data_col='fracs')

	for i, (x, h, s, p) in enumerate(zip(ind_arr, mean_arr, err_arr, p_val_arr)):
		add_stat_anno(ax[1],
					  x_arr=x,
					  y_arr=np.array(h) + np.array(s),
					  p_val_arr=p,
					  bwidth=.5*bar_width,
					  delta=0,
					  show_brackets=False,
