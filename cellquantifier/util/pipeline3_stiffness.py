...
class Pipeline3():
	...
	def check_det(self):
		...
		for ind in check_frame_ind:
			...
			# 		max_sig=self.config.MAX_SIGMA_2ND,
			# 		num_sig=self.config.NUM_SIGMA_2ND,
			# 		blob_thres=self.config.THRESHOLD_2ND,
			# 		peak_thres_rel=self.config.PEAK_THRESH_REL_2ND,
			# 		r_to_sigraw=self.config.R_TO_SIGRAW_2ND,
			# 		pixel_size=self.config.PIXEL_SIZE,
			#
			# 		diagnostic=True,
			# 		pltshow=True,
	...
	def track_simple(self):
		det_df = pd.read_csv(self.config.OUTPUT_PATH + self.config.ROOT_NAME + '-detData.csv')

		det_df = det_df.drop('r', axis=1)
		det_df['r'] = det_df['sig_raw'] * self.config.R_TO_SIGRAW
	...
	def plot_traj_simple(self):
        ...
        #             )
		#
		# particles = phys_df['particle'].unique()
		# for particle_num in particles:
		# 	traj = phys_df[phys_df['particle'] == particle_num]
	...
	def anim_traj_simple(self):
		...
		phys_df = relabel_particles(phys_df, col1='particle', col2='particle')
		if len(phys_df.drop_duplicates('particle')) > 1:
			# particle_max = phys_df['particle'].quantile(0.999)
			# particle_min = phys_df['particle'].quantile(0.001)
			particle_max = phys_df['particle'].max()
			particle_min = phys_df['particle'].min()
			particle_range = particle_max - particle_min
			cb_min=particle_min
